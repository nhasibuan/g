...
### v10.16 Compile-Fix Review (2026-07-29)

A MetaEditor compile report found five API/signature errors from the first v10.16 implementation pass:

| Error | Root Cause | Fix |
|---|---|---|
| `'Init' - wrong parameters count` | `CAdxFilter::Init()` was changed to four parameters while older call expectations and MQL4 compatibility were safer with the original three-parameter API. | Restored `Init(bool enabled, int period, double threshold)` and added `SetHysteresis(double)` as a separate setter. |
| `'Value' - wrong parameters count` | `Value(int shift=1)` replaced the original no-arg `Value()` API, while existing display/log code still expects no-arg access. | Restored no-arg `Value()` and added `ValueAt(int barShift)` for shifted closed-bar ADX reads. |
| `'m_hysteresis' - undeclared identifier` | Coupled to the failed method-signature parse path in MetaEditor. | Kept `m_hysteresis` as a class field and assigned it through `SetHysteresis()`. |
| `'shift' - undeclared identifier` | The shifted ADX accessor used the generic parameter name `shift`, which produced a compile diagnostic in the user's MetaEditor context. | Renamed the shifted accessor parameter/local loop variable to `barShift`. |

**Post-fix API contract:** Existing callers can keep using `m_adx.Value()` and `m_adx.Init(enabled, period, threshold)`. v10.16-specific code uses `m_adx.SetHysteresis(InpAdxHysteresis)` and `m_adx.ValueAt(2)` where shifted ADX is required.
